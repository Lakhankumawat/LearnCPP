...
- Traversing a binary tree is the process of visiting each node in the tree exactly once in a systematic way. Since a tree is a non linear data structure, therefore it can be traversed in various ways.
- There are three approaches to do that:
  1. Preorder: First the root node is accessed, then the left subtree and finally the right subtree
  2. Postorder: First the left subtree of a node is accessed, then the right subtree and finally the root node
  3. Inorder: First the left subtree of a node is accessed, then the root node and lastly the right subtree is accessed

![traversal](https://www.helpmestudybro.com/wp-content/uploads/2021/01/TreeTraversal.png
