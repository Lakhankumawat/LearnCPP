...
We express these bounds in terms of a fixed integer t>=2 called the minimum degree of the B-tree:
    ...
    Every internal node other than the root thus has at least t children. If the tree is nonempty, the root must have at least one key.
...
B-trees to implement many dynamic-set operations by below time complexity
Insertion - O(log n)
Search - O(log n)
Traverse - O(n)
*/
